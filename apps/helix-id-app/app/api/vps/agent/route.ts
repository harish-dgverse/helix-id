...
export async function GET(
  ...
  try {
    ...
    const external_url = url.searchParams.get("external_url") || undefined


    // Find agent
    const agents = getAgents()
    console.log("host", external_url)
    const agent = agents.find((a: any) => a.externalApiBaseUrl.includes(external_url) && a.externalApiKey === apiKey)
    ...
    logActivity({
      type: "VP_ISSUED",
      description: `VP issued for agent DID ${agent.did.slice(
        0,
        20
      )}... (On-demand)`,
      metadata: {
